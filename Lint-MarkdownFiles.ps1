...
param(
    [string]$Path = ".",
    [switch]$Detailed,
    [string]$ReportFile = "lint_report.txt"
)
...
function Write-ValidationResult {
    ...
    elseif ($Result.IsValid -and $Result.Warnings.Count -gt 0) {
        Write-Host "⚠ $relativePath - VALID WITH WARNINGS" -ForegroundColor Yellow
        if ($Detailed) {
        ...
        Write-Host "✗ $relativePath - INVALID" -ForegroundColor Red
        if ($Detailed) {
...
Write-Host "  ⚠ Files with warnings: $warningCount" -ForegroundColor Yellow
